...
- **"My `agent-browser-pool` call is slow but eventually connects."** The pool's TCP port range is likely
  **exhausted** (lanes themselves are unbounded — many busy lanes). It self-heals — it reaps dead owners
  and keeps retrying for up to `AGENT_BROWSER_POOL_WAIT` (default 600s). Do **not** try to "fix" this
